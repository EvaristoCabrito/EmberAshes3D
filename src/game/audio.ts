...
export const sfxPlay = {
  ...
  // own activation cue) share ATT01Blunt.mp3 — a real recording standing in for what used
  // to be separate synthesised bleeps. arrowAttack/magicAttack deliberately keep their own
  // synth cues below instead: ATT01Blunt reads as a melee/blunt impact, which is wrong for
  // an arrow loosing or a mage's ranged counter-bolt, so those two were pulled back out of
  // this bucket per direct report. spell and heal (stepSpell's cast cue, magic or
  // physical-skill alike) share Spellcast01.mp3 the same way. playSfxFileExclusive keeps
  // repeats of either file from stacking into a buzz when several fire in quick succession.
